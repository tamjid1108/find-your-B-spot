...
def main():
    ...
    for e in successful_tech_startups[:30]:
        loc = e['office_1_location']
        if loc!={}:
            near_companies = getCompaniesNear(loc['coordinates'][0],
                                        loc['coordinates'][1],2000)
        aux = []
    ...
    old_companies = list(coll.find({'$and':[{'deadpooled_year': np.nan},{'founded_year':{'$lte':2022-inputyears}}]}))

    not_possible_offices_criterion_2 = []
    for e in old_companies[:15]:
        loc = e['office_1_location']
        if loc!={}:
            near_companies = getCompaniesNear(loc['coordinates'][0],
                                        loc['coordinates'][1],2000)
        for f in near_companies[:15]:
            if f['_id'] != e['name']:
                not_possible_offices_criterion_2.append(f['_id'])

    not_possible_offices_c2 = list(set(not_possible_offices_criterion_2))
    print(len(not_possible_offices_c2))
    ########################################################################################################
    ...
    input_starbucks()
    starbucks_list = venuesListByQuery(possible_offices_c1_c2_coords, possible_offices_c1_c2,
                                    'starbucks', 1000)
    ...
    ######################################################################################################
    print(len(possible_offices_c1_c2_c3_c4_c5_c6_c7_coords))
    if len(possible_offices_c1_c2_c3_c4_c5_c6_c7_coords) < 1:
        ...
        for i in range(len(df)):
            if df['_id'][i] in possible_offices_c1_c2_c3_c4_c5_c6_c7:
                indexs.append(i)
        ...
        inputmoney, df_filtered.iloc[0][8], df_filtered.iloc[0][9], df_filtered.iloc[0][12], df_filtered.iloc[0][13], df_filtered.iloc[0][16], df_filtered.iloc[0][17], df_filtered.iloc[0][20])
